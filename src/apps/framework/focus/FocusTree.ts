...
export class FocusTree {
  ...
  unregisterFocusable(path: FocusPath) {
    ...
    if (this.#highlightedPath && isAncestorFocusPath(path, this.#highlightedPath)) {
      ...
      this.#highlightedPath = this.#findNearestAncestorNavigablePath(this.#highlightedPath, scopePath, false)
    }
    this.#hasPendingStructuralChanges = true
  }
  ...
  handleEscape() {
    ...
    if (parentPath && !(scopeFocusable?.input.trap && sameFocusPath(parentPath, session.activeScopePath))) {
  ...
  #resolveRequestedLogicalPath(requestPath: FocusPath, reason: FocusApplyReason): FocusPath | undefined {
  ...
  #chooseInitialHighlightedPath(activeScopePath: FocusPath): FocusPath | undefined {
    ...
    if (this.#highlightedPath && this.#isPathNavigableInScope(this.#highlightedPath, activeScopePath)) {
  ...
  #collectMeasuredNavigableFocusables(activeScopePath: FocusPath, clipToViewport: boolean): MeasuredFocusNode[] {
  ...
  #isPathNavigableInScope(path: FocusPath, activeScopePath: FocusPath): boolean {
    ...
    if (!sameFocusPath(activeScopePath, ROOT_FOCUS_PATH) && !isAncestorFocusPath(activeScopePath, path)) {
      return false
    }
  ...
  #findNearestAncestorFocusablePath(path: FocusPath, activeScopePath: FocusPath): FocusPath | undefined {
    for (let length = path.length - 1; length >= activeScopePath.length; length -= 1) {
      const ancestorPath = path.slice(0, length)
      if (!sameFocusPath(activeScopePath, ROOT_FOCUS_PATH) && !isAncestorFocusPath(activeScopePath, ancestorPath)) {
        continue
      }
