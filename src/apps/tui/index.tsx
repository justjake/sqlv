...
function SettingsModal(props: { onClose: () => void; termHeight: number; termWidth: number }) {
...
function RecentQueryViewBody(props: {
  ...
  useEffect(() => {
    ...
    const query = queries.find((candidate) => candidate.id === (selectedQueryId ?? currentQuery?.id)) ?? currentQuery
    if (!query) {
      return
    }
    queueMicrotask(() => {
    ...
    <box flexDirection="column" position="relative">
      <Focusable focusableId={RECENT_QUERY_AREA_ID}>
        <QueryListTable
          ...
          isRowFocused={(row) =>
            row.kind === "query" ? sameFocusPath(queryPaths.get(row.query.id), focusedDescendantPath) : false
          }
...
function buildRecentQueries(
  ...
  const selectedFinishedQuery = selectedQueryId
    ? finishedQueries.find(
        (query) =>
          query.id === selectedQueryId && !visibleFinishedQueries.some((candidate) => candidate.id === selectedQueryId),
      )
    : undefined
...
if (import.meta.main) {
  const engine = await createBunSqlVisor({ app: "tui" })
