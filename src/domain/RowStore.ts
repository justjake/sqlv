...
export type RowStore<Row extends BaseRow> = {
  list<T extends Row = Row>(): Promise<T[]>
...
export function rowDispatcher<T extends BaseRow>(
  ...
  return {
    list: async <T2 extends T>() => {
      const rows = await dispatch({ type: "list" } as RowAction<T2>)
      return mustBeArray(rows)
    },
