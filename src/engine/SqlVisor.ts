...
import { createEmptyEditorState, type EditorState } from "../model/editor/state"
import { replaceTextRange, type EditorRange } from "../model/editor/text"
import { formatQueryText } from "../model/formatQuery"
...
import { EpochMillis, type LogEntry, type QueryExecution, type QueryFlow, type Session } from "../model/Log"
import type { ObjectInfo } from "../model/objects"
import { OrderString } from "../model/Order"
import { pendingQueryState, queryStateOrPending, type QueryState } from "../model/QueryState"
...
import { unsafeRawSQL } from "../model/SQL"
...
type AppStateRowMap = Partial<Record<AppStateId, AppStateRow>>
...
export class SqlVisor {
  static async create(options: SqlVisorCreateOptions): Promise<SqlVisor> {
    ...
    await storage.storage.log.upsert(storage.session)
    ...
    await engine.#loadPersistedAppState()
  ...
  #appStateRows: AppStateRowMap = {}
  #state: SqlVisorState

  private constructor(args: {
    registry: AdapterRegistry
    storage: SqlVisorStorage["storage"]
    queryClient: QueryClient
    session: SqlVisorStorage["session"]
    suggestionProviders: SuggestionProvider[]
  }) {
  ...
  async refreshConnections(): Promise<Connection<any>[]> {
    const connections = await this.#queryClient.fetchQuery({
      ...
      queryFn: async () => sortConnections((await this.storage.connections.list()) as Connection<any>[]),
      retry: false,
      staleTime: 0,
    })
  ...
  async #loadPersistedHistory(): Promise<void> {
    const logEntries = (await this.storage.log.list()) as LogEntry[]
    ...
    this.#setState({
      history,
      queryExecution: selectedQueryExecutionId
        ? queryExecutionStateFromExecution(history.find((entry) => entry.id === selectedQueryExecutionId)!)
        : pendingQueryState(),
  ...
  async #loadPersistedSavedQueries(): Promise<void> {
    const savedQueries = (await this.storage.savedQueries.list()) as SavedQuery[]
    this.#setState({ savedQueries: sortSavedQueries(savedQueries) })
  }

  async #loadPersistedSettings(): Promise<void> {
    const rows = (await this.storage.settings.list()) as AnySettingsRow[]
    this.#replaceSettingsRows(rows)
  }

  async #loadPersistedAppState(): Promise<void> {
    const rows = (await this.storage.appState.list()) as AppStateRow[]
    this.#replaceAppStateRows(rows)
  }
  ...
  async addConnection<P extends Protocol>(input: AddConnectionInput<P>): Promise<Connection<ProtocolConfig<P>>> {
    ...
    await this.storage.connections.upsert(connection)
  ...
  async deleteConnection(connectionId: string): Promise<void> {
    ...
    await this.storage.connections.delete({
      id: connection.id,
      type: "connection",
    })
  ...
  async saveQueryAsNew(input: SaveQueryAsNewInput): Promise<SavedQuery> {
    ...
    await this.storage.savedQueries.upsert(savedQuery)
  ...
  async replaceSettings<Id extends SettingsId>(id: Id, settings: SettingsSchema[Id]): Promise<SettingsSchema[Id]> {
    ...
    const persistedRow = (await this.storage.settings.upsert(nextRow as AnySettingsRow)) as SettingsRow<Id>
    this.#applySettingsRow(persistedRow)
    return persistedRow.settings
  }

  getAppState<Value extends Json = Json>(id: string): Value | undefined {
    return this.#state.appState[id] as Value | undefined
  }

  getAppStateOrDefault<Value extends Json>(id: string, fallback: Value): Value {
    return (this.getAppState<Value>(id) ?? fallback) as Value
  }

  async updateAppState<Value extends JsonObject>(id: string, patch: Partial<Value>, fallback: Value): Promise<Value> {
    return this.replaceAppState(id, {
      ...this.getAppStateOrDefault(id, fallback),
      ...patch,
    } as Value)
  }

  async replaceAppState<Value extends Json>(id: string, value: Value): Promise<Value> {
    const now = EpochMillis.now()
    const currentRow = this.#appStateRows[id] as AppStateRow<Value> | undefined
    const nextRow: AppStateRow<Value> = {
      createdAt: currentRow?.createdAt ?? now,
      id,
      type: "appState",
      updatedAt: currentRow ? now : undefined,
    ...
    const persistedRow = (await this.storage.appState.upsert(nextRow as AppStateRow)) as AppStateRow<Value>
    this.#applyAppStateRow(persistedRow)
    return persistedRow.value
  }
  ...
  restoreQueryExecution(executionId: string) {
    ...
    this.#applySelectedConnection(restoredConnectionId, {
      persistSelection: restoredConnectionId !== undefined,
    })
    this.closeEditorCompletion()
  ...
  getQueryState(query: QueryRef): QueryExecutionState {
    ...
    const execution = historyEntry ?? (state.error instanceof QueryExecutionError ? state.error.execution : undefined)
    if (!execution) {
      return state
    }
    ...
    void queryPromise
      ...
      .catch(async (_error) => {
        ...
        if (!(error instanceof QueryExecutionError)) {
          try {
            await this.storage.log.upsert(execution)
          } catch {
            // keep in-memory history usable even if storage fails
          }
        }
  ...
  async #getQueryRunner(connection: Connection<any>): Promise<QueryRunnerImpl<any>> {
    ...
    const queryRunner = new QueryRunnerImpl(this.session, connection, executor, this.storage.log)
  ...
  #applySelectedConnection(
    ...
    if (options.persistSelection ?? true) {
      this.#persistSelectedConnection(connectionId)
    }
  ...
  #replaceState(
    patch: Pick<SqlVisorState, "connections" | "connectionSuggestions" | "queryExecution" | "objectsByConnectionId">,
  ) {
    this.#state = this.#normalizeState({
      ...this.#state,
      ...patch,
  ...
  #resolveSelectedConnectionId(connections: Connection<any>[]): string | undefined {
    ...
    const persistedConnectionId = this.#state.settings.workspace.lastSelectedConnectionId
  ...
  #persistSelectedConnection(connectionId: string | undefined) {
    const lastSelectedConnectionId = connectionId ?? ""
    if (this.#state.settings.workspace.lastSelectedConnectionId === lastSelectedConnectionId) {
      return
    }

    void this.replaceSettings("workspace", {
  ...
  #replaceAppStateRows(rows: AppStateRow[]) {
    const appStateRows = {} as AppStateRowMap
    const appState = defaultAppState()

    for (const row of rows) {
      appStateRows[row.id] = row
      appState[row.id] = row.value
    }

    this.#appStateRows = appStateRows
    this.#setState({ appState })
  }

  #applyAppStateRow(row: AppStateRow) {
    this.#appStateRows = {
      ...this.#appStateRows,
      [row.id]: row,
    }
  ...
  #createSuggestionContext() {
    return {
      getConnectionObjects: (connectionId: string) => this.#state.objectsByConnectionId[connectionId]?.data,
      listConnections: () => this.#state.connections.data ?? [],
      loadConnectionObjects: (connectionId: string) => this.loadConnectionObjects(connectionId),
    }
  }
}
...
function sortConnections(connections: Connection<any>[]): Connection<any>[] {
  return connections.toSorted((a, b) => b.createdAt - a.createdAt)
}
...
function replaceHistoryExecution(history: QueryExecution[], execution: QueryExecution): QueryExecution[] {
  ...
  return nextHistory.some((entry) => entry.id === execution.id)
    ? nextHistory
    : prependHistoryExecution(history, execution)
}
...
function partialConfigMatches(actual: unknown, partial: unknown): boolean {
  ...
  return Object.entries(partial).every(([key, value]) =>
    partialConfigMatches((actual as Record<string, unknown>)[key], value),
  )
}
