...
export type BorderChars = {
  ...
  topTee: string; bottomTee: string
  leftTee: string; rightTee: string
  cross: string
}
...
function normalizeBorder(input: TableProps<any>["border"]): BorderConfig {
  ...
  if (input === "header")
    return { top: true, bottom: true, left: true, right: true, header: true, columns: true }
  return input
}
...
function hline(
  ...
  const [cap0, cap1, tee, fill] =
    position === "top"    ? [chars.topLeft, chars.topRight, chars.topTee, chars.top] as const :
    position === "bottom" ? [chars.bottomLeft, chars.bottomRight, chars.bottomTee, chars.bottom] as const :
